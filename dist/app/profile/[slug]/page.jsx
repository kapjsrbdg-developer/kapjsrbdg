...
async function ProfilePage({ params }) {
    ...
    const karyawan = karyawan_json_1.default.find((employee) => employee.slug === slug);
        ...
        <div className="bg-white/95 backdrop-blur-sm rounded-2xl shadow-2xl p-8 mb-8 animate-fade-in border border-slate-200/50">
          <div className="flex flex-col md:flex-row items-center md:items-start gap-8">
            ...
            <div className="w-48 h-48 relative rounded-full overflow-hidden shadow-2xl border-4 border-white ring-4 ring-blue-900/20">
              <ProfileImage_1.default src={karyawan.foto || '/img/karyawan/placeholder.svg'} alt={`Foto ${karyawan.name}`} width={192} height={192} className="w-full h-full object-cover" priority/>
            </div>
        ...
        <div className="grid md:grid-cols-2 gap-8">
          ...
          <div className="bg-white/95 backdrop-blur-sm rounded-2xl shadow-2xl p-6 animate-fade-in border border-slate-200/50" style={{ animationDelay: '0.2s' }}>
            ...
            <div className="space-y-4">
              {karyawan.education.map((edu, index) => (<div key={index} className="border-l-4 border-blue-900 pl-4 py-3 bg-blue-50/50 rounded-r-lg backdrop-blur-sm">
          ...
          <div className="bg-white/95 backdrop-blur-sm rounded-2xl shadow-2xl p-6 animate-fade-in border border-slate-200/50" style={{ animationDelay: '0.4s' }}>
            ...
            <div className="space-y-4">
              {karyawan.certification.map((cert, index) => (<div key={index} className="border-l-4 border-green-600 pl-4 py-3 bg-green-50/50 rounded-r-lg backdrop-blur-sm">
...
async function generateStaticParams() {
    return karyawan_json_1.default.map((employee) => ({
        slug: employee.slug,
    }));
}
